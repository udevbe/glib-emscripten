...
static GIInfoType
get_parent_info_type (GIBaseInfo *info)
{
  if (!info->container)
    return GI_INFO_TYPE_INVALID;

  return gi_base_info_get_info_type (info->container);
}
...
static GICallableInfo *
get_callable_info_for_index (GIBaseInfo *rinfo, unsigned int index)
{
  if (!rinfo->container)
    {
...
{
  if (!gi_callable_info_has_async (info))
    return NULL;
...
{
  if (!gi_callable_info_has_sync (info))
    return NULL;
...
{
  if (!gi_callable_info_has_finish (info))
    return NULL;
 ...
 * [type@GLib.AsyncReadyCallback] parameter and user data.
 *
...
static gboolean
gi_callable_info_has_sync_or_async (GICallableInfo *info)
{
  GIBaseInfo *rinfo = (GIBaseInfo *) info;
  ...
  switch (gi_base_info_get_info_type (rinfo))
    ...
    case GI_INFO_TYPE_FUNCTION:
      {
        FunctionBlob *blob = GET_BLOB (FunctionBlob, rinfo);
        if (blob->sync_or_async == ASYNC_SENTINEL)
          return FALSE;
  ...
  GIInfoType parent_type = get_parent_info_type (rinfo);

  return parent_type == GI_INFO_TYPE_OBJECT || parent_type == GI_INFO_TYPE_INTERFACE;
}

/**
 * gi_callable_info_has_sync
 * @info: a callable info structure
 *
 * Gets whether a callable has a synchronous version.
 *
 * Returns: true if the callable has a sync version
 * Since: 2.80
 */
gboolean
gi_callable_info_has_sync (GICallableInfo *info)
{
  return gi_callable_info_is_async (info) && gi_callable_info_has_sync_or_async (info);
}

/**
 * gi_callable_info_has_async
 * @info: a callable info structure
 *
 * Gets whether a callable has an asynchronous version.
 *
 * Returns: true if the callable has an async version
 * Since: 2.80
 */
gboolean
gi_callable_info_has_async (GICallableInfo *info)
{
  return !gi_callable_info_is_async (info) && gi_callable_info_has_sync_or_async (info);
}

/**
 * gi_callable_info_has_finish
 * @info: a callable info structure
 *
 * Gets whether a callable has a finish version.
 *
 * Returns: true if the callable has a finish version
 * Since: 2.80
 */
gboolean
gi_callable_info_has_finish (GICallableInfo *info)
{
  if (!gi_callable_info_is_async (info))
    return FALSE;
