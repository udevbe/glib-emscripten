                                                    ...
                                                    const void        *unused)
{
  ...
  gi_base_info_unref ((GIBaseInfo *) finish_info);
                                                    ...
                                                    const void        *unused)
{
  ...
  g_assert_true (gi_callable_info_has_async (callable_info));
  g_assert_false (gi_callable_info_has_sync (callable_info));
  g_assert_false (gi_callable_info_has_finish (callable_info));

  GICallableInfo *async_info = gi_callable_info_get_async_function (callable_info);

  g_assert_false (gi_callable_info_has_async (async_info));
  g_assert_true (gi_callable_info_has_sync (async_info));
  g_assert_true (gi_callable_info_has_finish (async_info));

  g_assert_cmpstr (gi_base_info_get_name ((GIBaseInfo *) async_info), ==, "load_contents_async");

  GICallableInfo *sync_info = gi_callable_info_get_sync_function (async_info);

  g_assert_true (gi_callable_info_has_async (sync_info));
  g_assert_false (gi_callable_info_has_finish (sync_info));
  g_assert_false (gi_callable_info_has_sync (sync_info));
